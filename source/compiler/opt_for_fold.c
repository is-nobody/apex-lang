...
// emit closed-form bytecode for `for i = a to b { ...; acc = acc + P(i) }`
// where a is a compile-time constant and b is a runtime expression, and the
// body is straight-line arithmetic so P is a polynomial with const coeffs.
// uses Faulhaber formulas: sum_{m=0}^{N-1} m^k for k = 0..3.
bool try_emit_runtime_poly_sum(CodeGenerator* cg, ASTNode* node) {
    if (!node->for_stmt.var_name) return false;
    if (!node->for_stmt.end) return false;
    if (node->for_stmt.condition) return false;
    if (node->for_stmt.step) {
        double sv;
        if (!try_fold_number(cg, node->for_stmt.step, &sv) || sv != 1.0) return false;
    ...
    if (!try_fold_number(cg, node->for_stmt.start, &a_val)) return false;
    double b_dummy;
    if (try_fold_number(cg, node->for_stmt.end, &b_dummy)) return false;  // const path handles it

    ASTNode* body = node->for_stmt.body;
    if (!body || (body->type != AST_BLOCK && body->type != AST_PROGRAM)) return false;
    if (body_assigns_name(body, node->for_stmt.var_name)) return false;

    const char* names[16]; int nv = 0;                          // unique lhs names in body
    const char* acc_name = NULL;
    ASTNode* acc_delta = NULL;
    ...
    for (int i = 0; i < body->block.statements->count; i++) {
        ...
        double c; ASTNode* rhs = NULL;
        VarKind k = classify_assign(cg, s, nm, &rhs, &c);
        if (k == VK_ACC_PLUS) {
            if (acc_name) return false;                          // more than one accumulator
            acc_name = nm;
            acc_delta = rhs;
        } else if (k != VK_SIMPLE) {
            return false;                                        // ACC_MUL not supported here
        }
    }
    if (!acc_name) return false;
    if (strcmp(acc_name, node->for_stmt.var_name) == 0) return false;

    int acc_slot = find_local_slot(cg, acc_name);
    if (acc_slot < 0) return false;
    int acc_reg = cg->locals.registers[acc_slot];

    SymBind env[16]; int en = 0;                                 // resolve simple intermediates
    for (int i = 0; i < nv; i++) {
        if (strcmp(names[i], acc_name) == 0) continue;
        int idx = find_unique_assign(body, names[i]);
        if (idx < 0) return false;
        ASTNode* s = body->block.statements->nodes[idx];
        SymF sv = sym_from_ast(cg, s->var_assign.value, node->for_stmt.var_name, env, en, body);
    ...
    SymF delta = sym_from_ast(cg, acc_delta, node->for_stmt.var_name, env, en, body);
    if (!delta.ok || delta.has_geo) return false;                // only pure polynomial
    if (delta.poly[0] == 0 && delta.poly[1] == 0 &&
        delta.poly[2] == 0 && delta.poly[3] == 0) return false;  // zero delta

    // shift: sum_{i=a}^{b} P(i) = sum_{m=0}^{N-1} Q(m), Q(m) = P(a + m)
    double sh[SV_MAX_DEG + 1] = {0};
    double apow[SV_MAX_DEG + 1]; apow[0] = 1.0;
    for (int k = 1; k <= SV_MAX_DEG; k++) apow[k] = apow[k - 1] * a_val;
    for (int k = 0; k <= SV_MAX_DEG; k++) {
        double ck = delta.poly[k];
    ...
    int end_reg = codegen_expression(cg, node->for_stmt.end);    // evaluate bound once
    int n_reg = alloc_register(cg);                              // N = b - a + 1
    if (a_val == 0.0) {
        emit(cg, INST(OP_ADD_IMM, n_reg, end_reg, 1), node->line);
    } else if (a_val == (int)a_val && a_val >= 0 && a_val <= 65535) {
        emit(cg, INST(OP_SUB_IMM, n_reg, end_reg, (int)a_val), node->line);
        emit(cg, INST(OP_ADD_IMM, n_reg, n_reg, 1), node->line);
    } else {
        int ar = alloc_register(cg);
        int ci = bytecode_add_number_constant(cg->chunk, a_val);
        emit(cg, INST(OP_LOAD_NUM, ar, ci, 0), node->line);
        emit(cg, INST(OP_SUB, n_reg, end_reg, ar), node->line);
        emit(cg, INST(OP_ADD_IMM, n_reg, n_reg, 1), node->line);
        free_register(cg, ar);
    }

    int result = alloc_register(cg);                             // sum of P over the range
    int tmp    = alloc_register(cg);

    // sh[0] * N
    if (sh[0] == 0.0) {
        emit(cg, INST(OP_LOAD_NUM_IMM, result, 0, 0), node->line);
    } else if (sh[0] == 1.0) {
        emit(cg, INST(OP_MOVE, result, n_reg, 0), node->line);
    } else if (sh[0] == (int)sh[0] && sh[0] >= 0 && sh[0] <= 65535) {
        emit(cg, INST(OP_MUL_IMM, result, n_reg, (int)sh[0]), node->line);
    } else {
        int ci = bytecode_add_number_constant(cg->chunk, sh[0]);
        emit(cg, INST(OP_LOAD_NUM, tmp, ci, 0), node->line);
        emit(cg, INST(OP_MUL, result, n_reg, tmp), node->line);
    }

    // sh[1] * N*(N-1)/2
    if (sh[1] != 0.0) {
        emit(cg, INST(OP_SUB_IMM, tmp, n_reg, 1), node->line);
        emit(cg, INST(OP_MUL, tmp, tmp, n_reg), node->line);
        emit(cg, INST(OP_DIV_IMM, tmp, tmp, 2), node->line);
        if (sh[1] == 1.0) {
            emit(cg, INST(OP_ADD, result, result, tmp), node->line);
        } else if (sh[1] == (int)sh[1] && sh[1] >= 0 && sh[1] <= 65535) {
            emit(cg, INST(OP_MUL_IMM, tmp, tmp, (int)sh[1]), node->line);
            emit(cg, INST(OP_ADD, result, result, tmp), node->line);
        } else {
            int ci = bytecode_add_number_constant(cg->chunk, sh[1]);
            int sc = alloc_register(cg);
            emit(cg, INST(OP_LOAD_NUM, sc, ci, 0), node->line);
            emit(cg, INST(OP_MUL, tmp, tmp, sc), node->line);
            emit(cg, INST(OP_ADD, result, result, tmp), node->line);
            free_register(cg, sc);
        }
    }
    ...
    if (sh[2] != 0.0) {
        emit(cg, INST(OP_SUB_IMM, tmp, n_reg, 1), node->line);
        emit(cg, INST(OP_MUL, tmp, tmp, n_reg), node->line);
        int two_n = alloc_register(cg);
        emit(cg, INST(OP_MUL_IMM, two_n, n_reg, 2), node->line);
        emit(cg, INST(OP_SUB_IMM, two_n, two_n, 1), node->line);
        emit(cg, INST(OP_MUL, tmp, tmp, two_n), node->line);
        emit(cg, INST(OP_DIV_IMM, tmp, tmp, 6), node->line);
        free_register(cg, two_n);
        if (sh[2] == 1.0) {
            emit(cg, INST(OP_ADD, result, result, tmp), node->line);
        } else if (sh[2] == (int)sh[2] && sh[2] >= 0 && sh[2] <= 65535) {
            emit(cg, INST(OP_MUL_IMM, tmp, tmp, (int)sh[2]), node->line);
            emit(cg, INST(OP_ADD, result, result, tmp), node->line);
        } else {
            int ci = bytecode_add_number_constant(cg->chunk, sh[2]);
            int sc = alloc_register(cg);
            emit(cg, INST(OP_LOAD_NUM, sc, ci, 0), node->line);
            emit(cg, INST(OP_MUL, tmp, tmp, sc), node->line);
            emit(cg, INST(OP_ADD, result, result, tmp), node->line);
    ...
    if (sh[3] != 0.0) {
        emit(cg, INST(OP_SUB_IMM, tmp, n_reg, 1), node->line);
        emit(cg, INST(OP_MUL, tmp, tmp, tmp), node->line);
        int n2 = alloc_register(cg);
        emit(cg, INST(OP_MUL, n2, n_reg, n_reg), node->line);
        emit(cg, INST(OP_MUL, tmp, tmp, n2), node->line);
        emit(cg, INST(OP_DIV_IMM, tmp, tmp, 4), node->line);
        free_register(cg, n2);
        if (sh[3] == 1.0) {
            emit(cg, INST(OP_ADD, result, result, tmp), node->line);
        } else {
            int ci = bytecode_add_number_constant(cg->chunk, sh[3]);
            int sc = alloc_register(cg);
            emit(cg, INST(OP_LOAD_NUM, sc, ci, 0), node->line);
            emit(cg, INST(OP_MUL, tmp, tmp, sc), node->line);
            emit(cg, INST(OP_ADD, result, result, tmp), node->line);
    ...
    // guard: N < 1 -> sum is zero (empty loop)
    int skip = emit(cg, INST(OP_JUMP_IF_GTE_IMM, 0, n_reg, 1), node->line);
    emit(cg, INST(OP_LOAD_NUM_IMM, result, 0, 0), node->line);
    PATCH_JUMP(cg, skip, bytecode_current_offset(cg->chunk));

    emit(cg, INST(OP_ADD, acc_reg, acc_reg, result), node->line);

    int vs = find_local_slot(cg, node->for_stmt.var_name);       // post-loop value of loop var
    if (vs >= 0 && !is_local_dead_after_current_stmt(cg, node->for_stmt.var_name)) {
        int vreg = cg->locals.registers[vs];
        emit(cg, INST(OP_ADD_IMM, vreg, end_reg, 1), node->line);   // i = b + 1
        cg->locals.const_known[vs] = false;                      // b is runtime
    }

    free_register(cg, result);
    free_register(cg, tmp);
    free_register(cg, n_reg);
